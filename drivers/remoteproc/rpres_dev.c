...
{
	return omap_device_scale(&pdev->dev, val);
}
...
struct device *omap4_get_fdif_device(void)
{
	...
	if (!dev)
	{
		printk("%s:%s:%d: Cant find device named: fdif\n", __FILE__,__FUNCTION__,__LINE__);
		return NULL;
	}
...
static int __init init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(rpres_data); i++) {
		...
		pr_err("HDmod for %s found!\n", rpres_data[i].name);
		rpres_data[i].oh = oh;

		if (rpres_data[i].get_dev) {
			pr_err("Get_DEV module defined!!\n");
			...
			pr_err("No GetDev module defined!!\n");
			pdev = omap_device_build("rpres", i, oh,
					&rpres_data[i],
					sizeof(struct rpres_platform_data),
					ohl, ohl_cnt, false);
			if (IS_ERR(pdev))
