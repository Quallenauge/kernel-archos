...
/*-------------------------------------------------------------------------*/
static int archos_twl6030_set_phy_clk(struct usb_otg *x, int on)
{
...
static int archos_twl6030_phy_init(struct usb_otg *x)
{
	/*
...
static void archos_twl6030_phy_shutdown(struct usb_otg *x)
{
	struct archos_twl6030_usb *twl;
	struct device *dev;
	struct twl4030_usb_data *pdata;
...
static void linkstat_work(struct work_struct *work)
{
	...
	dev_err(&pdev->dev, ">>linkstat_work()\n");
...
printk("STATE_UNKNOWN -> STATE_NONE\n");
		...
		atomic_notifier_call_chain(&twl->phy.notifier, USB_EVENT_NONE,
				twl->otg.gadget);
	}
...
printk("STATE_VBUS -> STATE_NONE\n");
		...
		if (!(hw_state & STS_USB_ID)) {
			dev_err(&pdev->dev, "linkstat_work(): %d: \n" __LINE__);
...
static irqreturn_t archos_twl6030_usb_irq(int irq, void *_twl)
{
	...
	dev_err(&pdev->dev, ">>archos_twl6030_usb_irq()\n");
...
{
	struct archos_twl6030_usb *twl = _twl;
	dev_err(&pdev->dev, "<<archos_twl6030_usbotg_irq()\n");
	schedule_delayed_work(&twl->work, 0);
	dev_err(&pdev->dev, ">>archos_twl6030_usbotg_irq()\n");
	return IRQ_HANDLED;
}

static int archos_twl6030_set_peripheral(struct usb_otg *x,
		struct usb_gadget *gadget)
{
	struct archos_twl6030_usb *twl
	dev_err(&pdev->dev, ">>archos_twl6030_set_peripheral()\n");;
...
static int archos_twl6030_enable_irq(struct usb_otg *x)
{
	struct archos_twl6030_usb *twl = xceiv_to_twl(x);
	dev_err(&pdev->dev, ">>archos_twl6030_enable_irq()\n");
...
static int archos_twl6030_set_vbus(struct usb_otg *x, bool enabled)
{
	struct regulator *vbus_draw;
	struct archos_twl6030_usb *twl = xceiv_to_twl(x);
...
static int archos_twl6030_set_power(struct usb_otg *x, unsigned int mA)
{
       struct archos_twl6030_usb *twl = xceiv_to_twl(x);
...
static int archos_twl6030_set_host(struct usb_otg *x, struct usb_bus *host)
{
	struct archos_twl6030_usb *twl;
