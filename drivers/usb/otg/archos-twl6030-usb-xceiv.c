/*
 * archos_twl6030_usb - TWL6030 USB transceiver, talking to OMAP OTG driver.
 *
...
/* In module archos_twl6030_MODULE_PM_RECEIVER */
...
#define	comparator_to_twl(x) container_of((x), struct archos_twl6030_usb, comparator)
#define switchdev_to_archos_xceiv(x)	container_of((x), struct archos_twl6030_usb, usb_switch);
/*-------------------------------------------------------------------------*/

int archos_twl6030_usb_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&notifier_list, nb);
}
EXPORT_SYMBOL_GPL(archos_twl6030_usb_register_notifier);

int archos_twl6030_usb_unregister_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&notifier_list, nb);
}
EXPORT_SYMBOL_GPL(archos_twl6030_usb_unregister_notifier);
...
static inline u8 archos_twl6030_readb(struct archos_twl6030_usb *twl, u8 module, u8 address)
{
	u8 data, ret = 0;
...
static int archos_twl6030_start_srp(struct phy_companion *comparator)
{
	struct archos_twl6030_usb *twl = comparator_to_twl(comparator);

	archos_twl6030_writeb(twl, TWL_MODULE_USB, 0x24, USB_VBUS_CTRL_SET);
	archos_twl6030_writeb(twl, TWL_MODULE_USB, 0x84, USB_VBUS_CTRL_SET);

	mdelay(100);
	archos_twl6030_writeb(twl, TWL_MODULE_USB, 0xa0, USB_VBUS_CTRL_CLR);

	return 0;
}

static void archos_twl6030_enable_ldo_input_supply(struct archos_twl6030_usb *twl,
		bool enable)
{
	u8 misc2_data = 0;

	misc2_data = archos_twl6030_readb(twl, TWL6030_MODULE_ID0, TWL6030_MISC2);
	misc2_data &= ~(VUSB_IN_PMID | VUSB_IN_VBAT);
	if (enable)
		misc2_data |= VUSB_IN_VBAT;
	archos_twl6030_writeb(twl, TWL6030_MODULE_ID0, misc2_data, TWL6030_MISC2);
}
...
static irqreturn_t archos_twl6030_usb_irq(int irq, void *_twl)
{
	...
	if (!(hw_state & STS_USB_ID)) {
		if (vbus_state & VBUS_DET) {
			...
			archos_twl6030_enable_ldo_input_supply(twl, true);
			...
			if (twl->prev_status != OMAP_MUSB_UNKNOWN) {
				...
				if (twl->asleep) {
					...
					archos_twl6030_enable_ldo_input_supply(twl,
									false);
					twl->asleep = 0;
				}
			}
...
static irqreturn_t archos_twl6030_usbotg_irq(int irq, void *_twl)
{
	...
	if (hw_state & STS_USB_ID) {
		...
		if (twl->errata & TWL6032_ERRATA_VBUS_IRQ_LOST_OPA_MODE) {
			...
			twl->vbus_enable = 0;
			archos_twl6030_writeb(twl, TWL_MODULE_MAIN_CHARGE,
...
exit:
	archos_twl6030_writeb(twl, TWL_MODULE_USB, USB_ID_INT_MASK,
		       USB_ID_INT_LATCH_CLR);

	return IRQ_HANDLED;
}

static int archos_twl6030_enable_irq(struct archos_twl6030_usb *twl)
{
	archos_twl6030_writeb(twl, TWL_MODULE_USB, 0x1, USB_ID_INT_EN_HI_SET);
	...
	archos_twl6030_usbotg_irq(twl->irq1, twl);
...
{
	struct archos_twl6030_usb *twl = container_of(data, struct archos_twl6030_usb,
								set_vbus_work);
...
static int archos_twl6030_set_vbus(struct phy_companion *comparator, bool enabled)
{
	struct archos_twl6030_usb *twl = comparator_to_twl(comparator);
...
static void archos_twl6030_get_vbus(struct archos_twl6030_usb *twl)
{
	u8 ctrl1_data;
	 ...
	 */
	ctrl1_data = archos_twl6030_readb(twl, TWL_MODULE_MAIN_CHARGE,
				   CHARGERUSB_CTRL1);
...
//static unsigned int archos_twl6030_get_usb_max_power(struct usb_otg *x)
//{
...
static int __devinit archos_twl6030_usb_probe(struct platform_device *pdev)
{
	...
	twl->comparator.set_vbus			= archos_twl6030_set_vbus;
	twl->comparator.start_srp			= archos_twl6030_start_srp;

	// Init Archos specific values
//	twl->otg.get_usb_max_power = archos_twl6030_get_usb_max_power;
	...
	twl->asleep = 0;
	archos_twl6030_enable_irq(twl);
	dev_info(&pdev->dev, "Initialized Archos TWL6030 USB module\n");
...
static struct platform_driver archos_twl6030_usb_driver = {
	...
	.driver		= {
		.name	= "archos_twl6030_usb_xceiv123",
		.owner	= THIS_MODULE,
	},
};
...
MODULE_AUTHOR("Archos S.A. Modified by Quallenauge");
