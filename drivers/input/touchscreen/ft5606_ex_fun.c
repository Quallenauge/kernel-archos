...
static unsigned char CTPM_FW[] = {
	#include "FT5606_ZX_V12_AC97_20120628_app.i"
};
		    ...
		    int writelen, char *readbuf, int readlen)
{
	...
	dev_err(&client->dev, "%s 0x%02x (%d).\n", __func__, writebuf[0], readlen);
	print_hex_dump_bytes(KERN_ERR, 0, writebuf, writelen);
	...
	print_hex_dump_bytes(KERN_ERR, 0, readbuf, readlen);
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	...
	if (reg_val[0] == 0x79 && reg_val[1] == 0x3)
		...
		dev_dbg(&client->dev, "[FTS] Step 3: FAIL CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
			 reg_val[0], reg_val[1]);
		return -EIO;
	}
					...
					const char *buf, size_t count)
{
	...
	retval = strict_strtoul(valbuf, 16, &v);
