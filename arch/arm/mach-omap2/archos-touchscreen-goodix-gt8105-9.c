/*
*    archos-touchscreen-gt8105-9.c : 09/05/2012
...
static struct regulator_consumer_supply tsp_vcc_consumer[] = {
	REGULATOR_SUPPLY("tsp_vcc", "4-0055"),
};
...
static void goodix_gt8105_archos_reset(int on_off)
{
	pr_debug("%s:%d\n", __func__, on_off);
...
static int goodix_gt8105_archos_get_irq_level(void)
{
...
static int goodix_gt8105_archos_demux_i2c(int demux)
{
	pr_debug("%s:%d\n", __func__, demux);
...
int __init archos_touchscreen_goodix_gt8105_init(struct goodix_gt8105_platform_data *pdata)
{
	...
	if (gpio_is_valid(conf->shtdwn_gpio)) {
		...
		ret = gpio_request(reset_gpio, "goodix_gt8105_ts_rst");
	...
	if (gpio_is_valid(conf->irq_gpio)) {
		...
		ret = gpio_request(irq_gpio, "goodix_gt8105_ts_irq");
	...
	pdata->reset = goodix_gt8105_archos_reset;
	pdata->get_irq_level = goodix_gt8105_archos_get_irq_level;
	pdata->demux_i2c = goodix_gt8105_archos_demux_i2c;
