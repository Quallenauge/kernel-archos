/*
 *    archos-touchscreen-ft5606-9.c : 31/05/2012
...
static struct regulator_consumer_supply tsp_vcc_consumer[] = {
	REGULATOR_SUPPLY("tsp_vcc", "4-0038"),
};
...
static void ft5606_archos_reset(int on_off)
{
	pr_debug("%s:%d\n", __func__, on_off);
...
static int ft5606_archos_get_irq_level(void)
{
...
static int ft5606_archos_demux_i2c(int demux)
{
	pr_debug("%s:%d\n", __func__, demux);
...
int __init archos_touchscreen_ft5606_init(struct focaltech_ft5606_platform_data *pdata)
{
	...
	if (gpio_is_valid(conf->shtdwn_gpio)) {
		...
		ret = gpio_request(reset_gpio, "ft5606_ts_rst");
	...
	if (gpio_is_valid(conf->irq_gpio)) {
		...
		ret = gpio_request(irq_gpio, "ft5606_ts_irq");
	...
	pdata->reset = ft5606_archos_reset;
	pdata->get_irq_level = ft5606_archos_get_irq_level;
	pdata->demux_i2c = ft5606_archos_demux_i2c;
