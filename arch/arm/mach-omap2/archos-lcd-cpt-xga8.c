...
static int panel_enable(struct omap_dss_device *disp)
{
	...
	if (!IS_ERR(lcd_1v8)){
		if (regulator_enable(lcd_1v8) != 0){
			pr_debug("Can't enable lcd_1v8 regulator!\n");
		}
	}
	if (!IS_ERR(lcd_vcc)){
		if (regulator_enable(lcd_vcc) != 0){
			pr_debug("Can't enable lcd_vcc regulator!\n");
		}
	}
	
	if (gpio_is_valid(display_gpio.lcd_pwon)){
		pr_debug("Set LCD power on!\n");
