...
static struct ocv_config ocv_cfg = {
	...
	.table = {
		3715,3750,3710,3795,3779,
		3784,3813,3823,3862,3881,3920,
		3965,4003,4057,4086,4135,4169,
		4130, 4200
/*
 // First attempt:
		3300,3347,3392,3437,3482,3572,
		3617,3630,3666,3710,3725,3735,
		3803,3867,3891,3905,3935,4086,
		4111
*/
/*
 //Original Blaze:
...
static void __init board_xceiv_init(void)
{
	if (( get_feature_tag(FTAG_HAS_DCIN, feature_tag_size(feature_tag_dcin)))) {
		board_twl6030_usb_data.name = "archos_usb_xceiv";
		pr_debug("board with dcin\n");
	}
}
...
//	panel_cpt_xga_8_preinit(&auo_wxga_10_dss_device);
	...
	omap_android_display_setup(&board_dss_data,
				   &dsscomp_config_auo_wxga_10_wuxga,
...
static void __init omap_tablet_reserve(void)
{
	...
	if (is512MbyteG9Model()){
		...
		omap_ram_console_init(OMAP_RAM_CONSOLE_START_1G,
				OMAP_RAM_CONSOLE_SIZE_1G);
	}
...
MACHINE_START(ARCHOS_A101S, "ARCHOS A101S board")
